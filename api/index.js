      ...
      if (utXOs.length == 0) {
          ...
          if (unlockedBalance > currentBalance) {
              ...
              if (unlockedBalance == currentAddress.deposit_amount) {
                ...
                const amountToSend = Number(unlockedBalance * .9).toFixed()
                const balance = Number(unlockedBalance - amountToSend).toFixed();

                 const tx = await MoneroService.sendTx(
                    currentAddress.destination_address,
                    amountToSend
                  )
