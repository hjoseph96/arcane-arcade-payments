...
class CentralWalletService {
            ...
            for (let i = 0; i < addressData.length; i++) {
                ...
                if (amountToRemove > 0) {
                    if (balance <= amountToRemove) {
                        ...
                        paymentOutputs.push({ address: target_address, amount: amountToSend });
                        ...
                        paymentOutputs.push({ address: target_address, amount: amountToSend });
                        paymentOutputs.push({ address: addrData.address, amount: returnAmount });
                        amountToRemove = 0;
                    }
    ...
    static async fetchOrCreateCentralAddress(coin_type) {
        const centralWalletId = await this.fetchCentralWalletId();
        const centralWallet = await database.Wallet.findOne({
            where: { id: String(centralWalletId)} ,
            include: [
                {
        ...
        if (centralWallet && centralWallet.BitcoinAddresses.length > 0) {
            return centralWallet.BitcoinAddresses[0]
        } else {
            const coins = Coinjs;
    ...
    static async fetchOrCreateCentralWallet() {
        const centralWallet = await database.Wallet.findOne({
            where: { user_id: null }
        });

        if (!centralWallet) {
            return await database.Wallet.create({ where: { user_id: null }})
        } else {
    ...
    static async calculatePlatformFee({ coin_amount, coin_type }) {
