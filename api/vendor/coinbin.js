$(document).ready(function() {
				...
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					if($("#walletSegwit").is(":checked")){
						if($("#walletSegwitBech32").is(":checked")){
							...
							var sw = coinjs.BitcoinAddress(pubkey);
							address = sw.address;
						}
	...
	$("#walletConfirmSend").click(function(){
		...
		if($("#walletSegwit").is(":checked")){
			if($("#walletSegwitBech32").is(":checked")){
				...
				var sw = coinjs.BitcoinAddress($("#walletKeys .pubkey").val());
			}
	...
	$("#newSegWitKeysBtn").click(function(){
		...
		if($("#newSegWitBech32addr").is(":checked")){
			...
			var sw = coinjs.BitcoinAddress(coin.pubkey);
		}

		$("#newBitcoinAddress").val(sw.address);
	...
	$("#newSegwitPaperwalletBtn").click(function(){
		...
		var paperwallet = window.open();
		paperwallet.document.write('<h2>BTC SegWit PaperWallet</h2><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Address (Share)</h3></div><div style="text-align: center;"><div id="qraddress"></div><p>'+$("#newBitcoinAddress").val()+'</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Public Key</h3></div><div style="text-align: center;"><div id="qrpubkey"></div><p>'+$("#newSegWitPubKey").val()+'</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Redeem Script</h3></div><div style="text-align: center;"><div id="qrredeem"></div><p>'+$("#newSegWitRedeemScript").val()+'</p></div></div><hr><div style="margin-top: 5px; margin-bottom: 5px"><div><h3 style="margin-top: 0">Private Key (KEEP SECRET!)</h3></div><div style="text-align: center;"><div id="qrprivkey"></div><p>'+$("#newSegWitPrivKey").val()+'</p></div></div>');
		paperwallet.document.close();
		paperwallet.focus();
		new QRCode(paperwallet.document.getElementById("qraddress"), {text: $("#newBitcoinAddress").val(), width: 110, height: 110});
	...
	function decodeRedeemScript(){
		...
		if(decode){
			...
			if(decode.type == "multisig__") {
				...
				$("#verifyRsData").removeClass("hidden");
				$("#verifyRsDataSegWit .BitcoinAddress").val(decode['address']);
